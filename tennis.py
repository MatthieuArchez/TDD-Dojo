...
def inc_sets(sets_score, winner_id):
    ...
    return ((sets_winner, sets_loser), is_match_over) if winner_id == 1 else ((sets_loser, sets_winner), is_match_over)
...
def inc_game(game_score, winner_id):
    ...
    is_set_over = True if ((game_winner == 6 and game_loser < 5) or (game_winner == 7 and game_loser in (5, 6))) else False

    return ((game_winner, game_loser), is_set_over) if winner_id == 1 else ((game_loser, game_winner), is_set_over)
